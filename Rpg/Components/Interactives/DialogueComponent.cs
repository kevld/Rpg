...
namespace Rpg.Components.Interactives
{
    public class DialogueComponent : InteractiveComponent
    {
        #region properties
        ...
        /// </summary>
        private Vector2 _characterSize = Vector2.Zero;
        ...
        /// Default constructor
        /// </summary>
        public DialogueComponent(Entity owner, IGraphicsService graphicsService, IContentService contentService) : base(owner, graphicsService)
        {
            _contentService = contentService;
            _characterSize = _contentService.DialogFont.MeasureString(new StringBuilder("W", 1));
            ...
            _fillTexture = new Texture2D(graphicsService.GraphicsDevice, 1, 1);
            _fillTexture.SetData(new[] { FillColor });

            _borderTexture = new Texture2D(graphicsService.GraphicsDevice, 1, 1);
            _borderTexture.SetData(new[] { BorderColor });

            _pages = new List<string>();
            _currentPage = 0;

            var sizeX = (int)(graphicsService.GraphicsDevice.Viewport.Width * 0.5);
            var sizeY = (int)(graphicsService.GraphicsDevice.Viewport.Height * 0.2);

            Size = new Vector2(sizeX, sizeY);

            var posX = Camera.Zero.X - (Size.X / 2f);
            var posY = Camera.Zero.Y + 480 / 2 - Size.Y - 5;
        ...
        public override void Update(GameTime gameTime)
        {
            if (Active)
            {
                ...
                if (InputManager.Instance.KeyPressed(Keys.Enter) || InputManager.Instance.KeyPressed(Keys.W))
                {
        ...
        public override void Draw(GameTime gameTime)
        {
            if (Active)
            {
                ...
                {
                    _graphicsService.SpriteBatch.Draw(_borderTexture, side, Color.White);
                }

                // Draw background fill texture (in this example, it's 50% transparent white)
                _graphicsService.SpriteBatch.Draw(_fillTexture, TextRectangle, Color.White * 0.5f);

                // Draw the current page onto the dialog box
                _graphicsService.SpriteBatch.DrawString(_contentService.DialogFont, _pages[_currentPage], TextPosition, DialogColor);
                ...
                if (BlinkIndicator() || _currentPage == _pages.Count - 1)
                {
                    ...
                    _graphicsService.SpriteBatch.DrawString(_contentService.DialogFont, ">", indicatorPosition, Color.Red);
                }
            }
        }
